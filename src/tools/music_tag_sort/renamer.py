...
def process_drive_folder(source_folder_id, dest_folder_id, separator) -> Dict[str, int]:
    ...
    for file in music_files:
        try:
            ...
            renamed_path = rename_music_file(temp_path, tempfile.gettempdir(), separator)
            summary["renamed"] += 1
            ...
            log.error(f"Failed processing Drive file: {file.get('name')}", exc_info=True)
            summary["failed"] += 1
...
def generate_filename(metadata: Dict[str, str], config: Dict) -> Optional[str]:
    ...
    for field in rename_order:
        ...
        if not value and field in required_fields:
            log.debug(f"Required field '{field}' is missing, skipping filename generation.")
            return None
