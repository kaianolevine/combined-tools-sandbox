...
def test_get_metadata_with_valid_mp3(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("tools.music_tag_sort.renamer.MP3", lambda f, ID3=None: DummyAudio(tags))
...
def test_rename_music_file_creates_dir(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "tools.music_tag_sort.renamer.get_metadata",
...
def test_rename_files_in_directory_full_flow(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "tools.music_tag_sort.renamer.get_metadata", lambda _: {"artist": "A", "title": "B"}
    )
    monkeypatch.setattr("tools.music_tag_sort.renamer.generate_filename", lambda m, c: "A__B.mp3")
    monkeypatch.setattr("os.rename", lambda s, d: None)

    res = renamer.rename_files_in_directory(str(tmp_path), {"rename_order": ["artist", "title"]})
...
def test_process_drive_folder_success(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "tools.music_tag_sort.renamer.drive.get_drive_service", lambda: mock_service
    )
    monkeypatch.setattr("tools.music_tag_sort.renamer.drive.list_music_files", lambda s, f: [fake])
    monkeypatch.setattr(
        "tools.music_tag_sort.renamer.drive.download_file", lambda s, i, p: open(p, "w").write("x")
    )
    monkeypatch.setattr("tools.music_tag_sort.renamer.rename_music_file", lambda p, o, s: p)
    monkeypatch.setattr("tools.music_tag_sort.renamer.drive.upload_file", lambda s, p, d: None)
...
def test_process_drive_folder_upload_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("tools.music_tag_sort.renamer.rename_music_file", lambda p, o, s: p)
    monkeypatch.setattr(
        "tools.music_tag_sort.renamer.drive.upload_file",
